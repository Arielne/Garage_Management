...
class InventoryImportExportDialog extends ConsumerStatefulWidget {
  ...
  const InventoryImportExportDialog({super.key, this.initialItem, this.onSuccess});

  @override
  ConsumerState<InventoryImportExportDialog> createState() => _InventoryImportExportDialogState();
}

class _InventoryImportExportDialogState extends ConsumerState<InventoryImportExportDialog> {
  ...
  Future<void> _submitTransaction(List<InventoryItem> items) async {
    ...
    final success = await repo.createTransaction(item, _transactionType, quantity, _noteController.text);
    ...
    return Dialog(
      ...
      child: Container(
        ...
        child: asyncItems.when(
          ...
          data: (items) {
            if (items.isEmpty) {
              return const Center(child: Text('Chưa có phụ tùng nào trong kho.'));
            }
                ...
                Container(
                  ...
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<String>(
                      isExpanded: true,
                      value: items.any((i) => i.id == _selectedItemId) ? _selectedItemId : items.first.id,
